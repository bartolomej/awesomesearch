...
const MetaService = require('./metadata');
...
describe('Website metadata parsing tests', function () {
  ...
  it('should parse node-js website html', async function () {
    ...
    `;
    const metadata = await metaService.parseHtml(mockData, 'https://glitch.com');
  ...
  it('should parse react-native website html', async function () {
    ...
    `;
    const metadata = await metaService.parseHtml(mockData, 'https://reactnative.dev');
    expect(metadata).toEqual({
      uid: 'reactnative.dev',
      author: null,
